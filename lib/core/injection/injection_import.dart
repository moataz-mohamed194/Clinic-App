...
Future<void> init() async {
  ...
  sl.registerFactory(() => AddUpdateGetFeesBloc(
      getFeesOfDay: sl(),
      getFeesOfMonth: sl(),
      addFees: sl(),
      updateFeesData: sl(),
      deleteFeesData: sl()));
  sl.registerFactory(() => AddGetDoctorBloc(getDoctor: sl(), addDoctor: sl()));
  sl.registerFactory(() => EyesCubit());
  sl.registerFactory(() => CheckBoxCubit());
  sl.registerFactory(() => BottomCubit());
  sl.registerFactory(
      () => LocaleCubit(changeLanguageUseCase: sl(), cubitLanguage: 'en'));

  sl.registerFactory(() => AddUpdateUserBloc(addUser: sl(), updateUser: sl()));
  sl.registerFactory(() => AddGetNurseBloc(addNurse: sl(), getNurse: sl()));
  sl.registerFactory(() => AddUpdateGetClinicBloc(
        getClinic: sl(),
        addClinic: sl(),
        updateClinic: sl(),
      ));
  sl.registerFactory(() => AddUpdateGetSickBloc(
        getSick: sl(),
        addSick: sl(),
        updateSick: sl(),
        getSickBasedOnUser: sl(),
        addSickReport: sl(),
        updateSickAsEntered: sl(),
      ));
  sl.registerFactory(() => VisitorBloc(getAllVisitors: sl()));
  sl.registerFactory(() => LoginBloc(loginMethod: sl()));
  sl.registerFactory(
      () => AddUpdateVisitorBloc(addVisitor: sl(), updateVisitor: sl()));
  ...
  sl.registerLazySingleton<FeesRepository>(
      () => FeesRepositoriesImpl(remoteDataSource: sl(), networkInfo: sl()));
  sl.registerLazySingleton<BaseLanguageRepository>(
      () => LanguageRepository(languageLocaleDataSource: sl()));
  sl.registerLazySingleton<DoctorRepository>(
      () => DoctorRepositoriesImpl(remoteDataSource: sl(), networkInfo: sl()));
  sl.registerLazySingleton<NurseRepository>(
      () => NurseRepositoriesImpl(remoteDataSource: sl(), networkInfo: sl()));
  sl.registerLazySingleton<ClinicRepository>(
      () => ClinicRepositoriesImpl(remoteDataSource: sl(), networkInfo: sl()));
  sl.registerLazySingleton<VisitorRepository>(
      () => VisitorRepositoriesImpl(remoteDataSource: sl(), networkInfo: sl()));
